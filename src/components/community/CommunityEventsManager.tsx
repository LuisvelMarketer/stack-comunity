...
export function CommunityEventsManager({ communityId }: CommunityEventsManagerProps) {
  ...
  const queryKey = ["community-events", communityId];
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => toast.error("Error al crear evento: " + error.message),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => toast.error("Error al actualizar evento: " + error.message),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => toast.error("Error al eliminar evento: " + error.message),
  });
  ...
  const generateRecurringMutation = useMutation({
    ...
    onError: (error) => toast.error("Error al generar eventos: " + error.message),
  });

  const resetForm = () => {
    setFormData({ title: "", description: "", event_date: "", location: "", max_attendees: "", recurrence_type: "", recurrence_end_date: "" });
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => generateRecurringMutation.mutate()} disabled={generateRecurringMutation.isPending}>
            ...
            <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <Input value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder="Nombre del evento" maxLength={100} />
                </div>
                <div>
                  <label className="text-sm font-medium">Descripción</label>
                  <Textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Descripción del evento" maxLength={500} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} placeholder="Ubicación o enlace" maxLength={200} />
                </div>
                <div>
                  <label className="text-sm font-medium">Máximo de asistentes</label>
                  <Input type="number" value={formData.max_attendees} onChange={(e) => setFormData({ ...formData, max_attendees: e.target.value })} placeholder="Sin límite" min="1" />
                </div>
                <div className="border-t pt-4 mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select value={formData.recurrence_type || "none"} onValueChange={(value) => setFormData({ ...formData, recurrence_type: value === "none" ? "" : value })}>
                    ...
                    {formData.recurrence_type && formData.recurrence_type !== "none" && (
                      <div>
                        ...
                        <Input type="datetime-local" value={formData.recurrence_end_date} onChange={(e) => setFormData({ ...formData, recurrence_end_date: e.target.value })} />
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <TableRow key={event.id} className={event.parent_event_id ? "opacity-70" : ""}>
                    <TableCell className="font-medium">
                      <div className="flex items-center gap-2">
                        {event.title}
                        {event.parent_event_id && <Badge variant="outline" className="text-xs">Instancia</Badge>}
                      </div>
                    </TableCell>
                    <TableCell>{format(new Date(event.event_date), "dd MMM yyyy HH:mm", { locale: es })}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex gap-2 justify-end">
                        ...
                        <Button size="sm" variant="destructive" onClick={() => {
                          ...
                          if (confirm(msg)) deleteMutation.mutate(event.id);
                        }}>
